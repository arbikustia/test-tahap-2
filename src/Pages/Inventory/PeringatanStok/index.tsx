...
import useFetchPeringatanStok from '@/Hooks/Inventory/PeringatanStokSlice';
...
const PeringatanStok = () => {
    ...
    const {
        ...
        queryParams,
        setQueryParams,
    } = useFetchPeringatanStok();


      const deleteData = async () => {
        try {
            await InventoryService.deletePeringatanStok(dataDelte.id);
        }finally { 
    ...
    const downloadExcel = async () => {
        ...
        try {
          const response = await InventoryService.dataPeringatanStokDownload(queryParams);
                        ...
                        <div className="w-full flex flex-col items-end gap-4 self-stretch rounded-xl bg-white-50 dark:bg-grey-700 p-4">
                            <div className="w-full flex justify-between items-center py-4">
                                <div className='flex  flex-col'>
                                    ...
                                    <span className=" text-black font-extrabold text-2xl">Peringatan Stok</span>
                                </div>
                                <div className='flex items-end justify-around gap-x-10'>
                                    ...
                                    <div className='text-blue-500'>
                                        <button onClick={() => navigate('/inventory/peringatan-stock/create')} className="font-bold rounded-md flex justify-center gap-2 items-center  bg-blue-500 text-white-50 w-60 h-12 hover:bg-blue-800">
                                ...
                                <table className="min-w-full bg-white">
                                    <thead className=" bg-white-500 dark:bg-grey-600">
                                        <tr className="">
                                            ...
                                            <th className="py-2 px-4 text-center">Peringatan Stok</th>
                                    ...
                                    <tbody className="bg-white-7000 text-white-10 bg-white-50 dark:bg-grey-700 ">
                                        {dataPeringatanStok.map((item: peringatanStokType, index: number) => (
                                            <tr key={`table-${index}`} className="border-gray-300 border-b ">
                                                ...
                                                <td className="py-4 px-4  text-left flex gap-x-2 justify-center items-center ">
                                                    <img
                                                        src={EditIcon}
                                                        onClick={() => navigate(`/inventory/peringatan-stock/create`, {
                                                            state: {
                                                                data: item
                                                            }
                                                        })}
