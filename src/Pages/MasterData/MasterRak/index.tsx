...
import useFetchMasterRak from '@/Hooks/Inventory/MasterRakSlice';
...
const MasterRak = () => {
    ...
    const deleteData = async () => {
        try {
            await MasterService.deleteMasterRak(dataDelte.id);
        }finally { 
                        ...
                        <div className="w-full flex flex-col items-end gap-4 self-stretch rounded-xl bg-white-50 dark:bg-grey-700 p-4">
                            <div className="w-full flex justify-between items-center py-4">
                                <div className='flex  flex-col'>
                                    ...
                                    <span className=" text-black font-extrabold text-2xl">Master Rak</span>
                                </div>
                                <div className='flex items-end justify-around gap-x-10'>
                                    ...
                                    <div className='text-blue-500'>
                                        <button onClick={() => navigate('/master/rak/create')} className="font-bold rounded-md flex justify-center gap-2 items-center  bg-blue-500 text-white-50 w-48 h-12 hover:bg-blue-800">
                                            <img src={Plus} alt="" /><span className='text-white-500'>Tambah Rak</span>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <table className="min-w-full bg-white">
                                    <thead className=" bg-white-500 dark:bg-grey-600">
                                        <tr className="">
                                            ...
                                            <th className="py-2 px-4 text-center">Nama Rak Layout</th>
                                    ...
                                    <tbody className="bg-white-7000 text-white-10 bg-white-50 dark:bg-grey-700 ">
                                        {dataMasterRak.map((item: masterRakType, index: number) => (
                                            <tr key={`table-${index}`} className="border-gray-300 border-b ">
                                                ...
                                                <td className="py-4 px-4  text-left flex gap-x-2 justify-center items-center ">
                                                    <img
                                                        src={EditIcon}
                                                        onClick={() => navigate(`/master/rak/create`, {
                                                            state: {
                                                                data: item
                                                            }
                                                        })}
                                                    ...
                                                    <img
                                                        ...
                                                        width={40}
                                                        height={40}
                                                        onClick={() => {setSuccessDelete(true), setDataDelete({name: item.no_rak, id:item.id})}}
                                                        alt="delete"
                                                        className="cursor-pointer hover:scale-105 transition-all p-1"
                                                    />
                                                </td>
