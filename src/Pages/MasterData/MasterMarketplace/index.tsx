...
import useFetchMasterMarketPlace from '@/Hooks/Inventory/MasterMarketPlaceSlice';
import { masterMarketPlaceType } from '@/Types/MasterData/MasterData.Type';



const MasterMarketplace = () => {
  ...
  return (
    <>
            ...
            <div className="w-full flex flex-col items-end gap-4 self-stretch rounded-xl bg-white-50 dark:bg-grey-700 p-4">
              <div className="w-full flex justify-between items-center py-4">
                <div className='flex  flex-col'>
                  ...
                  <span className=" text-black font-extrabold text-2xl">Master Market Place</span>
                </div>
                <div className='flex items-end justify-around gap-x-10'>
                  ...
                  <div className='text-blue-500'>
                    <button onClick={() => navigate('/master/market-place/create')} className="font-bold rounded-md flex justify-center gap-2 items-center  bg-blue-500 text-white-50 w-48 h-12 hover:bg-blue-800">
                      <img src={Plus} alt="" /><span className='text-white-500'>Tambah Market Place</span>
                    </button>
                  </div>
                </div>
                ...
                <table className="min-w-full bg-white">
                  <thead className=" bg-white-500 dark:bg-grey-600">
                    <tr className="">
                      ...
                      <th className="py-2 px-4 text-center">Nama Market Place</th>
                  ...
                  <tbody className="bg-white-7000 text-white-10 bg-white-50 dark:bg-grey-700 ">
                    {dataMasterMarketPlace.map((item: masterMarketPlaceType, index: number) => (
                      <tr key={`table-${index}`} className="border-gray-300 border-b ">
                        <td className="py-2 px-4 text-center">
                          {index + 1}
                        </td>
                        <td className="py-2 px-4 text-center">
                          {item.nama_market_place}
                        </td>
                        <td className="py-2 px-4 text-center">
                          {item.last_update}
                        </td>
      ...
      <ModalWrapper modalOpen={successDelete} setModalOpen={setSuccessDelete}>
        <div className='h-96 w-full grid grid-rows-2 absolute -top-20 rounded-lg'>
